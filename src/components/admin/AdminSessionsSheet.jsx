...
export function AdminSessionsSheet() {
  const [sessionsData, setSessionsData] = useState({ headers: [], rows: [], total: 0 });
  const [availabilityData, setAvailabilityData] = useState({ headers: [], rows: [], total: 0 });
  ...
  const fetchData = useCallback(async (showLoader = false) => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE_URL}/admin-data/sessions`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      const data = await res.json();
      if (data.success) {
        setSessionsData(data.sessions || { headers: [], rows: [], total: 0 });
        setAvailabilityData(data.availability || { headers: [], rows: [], total: 0 });
        setLastUpdated(new Date());
      } else { setError(data.error || 'Failed to fetch sessions data'); }
  ...
  useEffect(() => { setSortCol(null); setSortAsc(true); setGlobalSearch(''); setColumnFilters({}); setActiveQuickFilters([]); setHiddenCols(new Set([0])); }, [activeTab]);
  ...
  const currentData = activeTab === 'sessions' ? sessionsData : availabilityData;
  const { headers, rows } = currentData;

  // Quick filters per tab
  // Sessions cols: 0=ID,1=Student,2=StuEmail,3=StuPhone,4=Counselor,5=CounEmail,6=Date,7=StartTime,8=EndTime,9=Duration,10=Type,11=Status,12=Notes,13=MeetLink,14=Created
  // Availability cols: 0=ID,1=Counselor,2=CounEmail,3=Date,4=StartTime,5=EndTime,6=Duration,7=Booked,8=Student,9=StuEmail,10=Created
  const quickFilters = activeTab === 'sessions' ? [
    { id: 'confirmed', label: 'Confirmed', icon: '✅', color: '#16a34a', col: 11, value: 'confirmed' },
    { id: 'completed', label: 'Completed', icon: '🏁', color: '#2563eb', col: 11, value: 'completed' },
    { id: 'cancelled', label: 'Cancelled', icon: '❌', color: '#dc2626', col: 11, value: 'cancelled' },
    { id: 'scheduled', label: 'Scheduled', icon: '📅', color: '#7c3aed', col: 11, value: 'scheduled' },
    { id: 'has-meet', label: 'Has Meet Link', icon: '🔗', color: '#0891b2', col: 13, match: v => v && v.startsWith('http') },
  ] : [
    { id: 'booked', label: 'Booked', icon: '📌', color: '#dc2626', col: 7, value: 'Yes' },
    { id: 'available', label: 'Available', icon: '🟢', color: '#16a34a', col: 7, value: 'No' },
  ];
  ...
  const filteredRows = useMemo(() => {
    return rows.filter(row => {
      ...
      for (const qfId of activeQuickFilters) { const qf = quickFilters.find(q => q.id === qfId); if (!qf) continue; const cv = (row[qf.col] || '').trim(); if (qf.match ? !qf.match(cv) : cv.toLowerCase() !== qf.value.toLowerCase()) return false; }
      return true;
  ...
  if (loading && rows.length === 0) return <div style={{ textAlign: 'center', padding: '60px 20px' }}><div className="spinner" style={{ margin: '0 auto 16px' }}></div><p style={{ color: 'var(--color-text-secondary)' }}>Loading sessions data...</p></div>;
  ...
  return (
    <div>
      <div className="page-header" style={{ marginBottom: '8px' }}>
        <h1 className="page-title">📅 Sessions & Availability Tracker</h1>
        <p className="page-subtitle">Live from database</p>
      </div>

      {/* Sub-tabs: Sessions / Availability */}
      <div style={{ display: 'flex', gap: '0', marginBottom: '16px', borderBottom: '2px solid #e2e8f0' }}>
        <button onClick={() => setActiveTab('sessions')}
          style={{ padding: '12px 24px', border: 'none', background: 'none', cursor: 'pointer', fontWeight: activeTab === 'sessions' ? '700' : '500',
            color: activeTab === 'sessions' ? '#2563eb' : '#64748b', borderBottom: activeTab === 'sessions' ? '3px solid #2563eb' : '3px solid transparent',
            marginBottom: '-2px', fontSize: '14px', transition: 'all 0.2s' }}>
          📋 Sessions ({sessionsData.rows.length})
        </button>
        <button onClick={() => setActiveTab('availability')}
          style={{ padding: '12px 24px', border: 'none', background: 'none', cursor: 'pointer', fontWeight: activeTab === 'availability' ? '700' : '500',
            color: activeTab === 'availability' ? '#16a34a' : '#64748b', borderBottom: activeTab === 'availability' ? '3px solid #16a34a' : '3px solid transparent',
            marginBottom: '-2px', fontSize: '14px', transition: 'all 0.2s' }}>
          🕐 Counselor Availability ({availabilityData.rows.length})
        </button>
      </div>
      ...
      <QueryToolbar headers={headers} columnFilters={columnFilters} setColumnFilters={setColumnFilters} globalSearch={globalSearch} setGlobalSearch={setGlobalSearch}
        ...
        resultCount={filteredRows.length} totalCount={currentData.total} lastUpdated={lastUpdated} onRefresh={() => fetchData(false)}
        onExport={() => exportCSV(headers, filteredRows, hiddenCols, activeTab === 'sessions' ? 'sessions' : 'availability')} hiddenCols={hiddenCols} toggleCol={toggleCol} />
            ...
            <tbody>
              {filteredRows.length === 0 ? (
                ...
                <tr key={rowIdx} style={{ borderBottom: '1px solid var(--color-border, #eee)' }}>
                  ...
                  {headers.map((_, colIdx) => hiddenCols.has(colIdx) ? null : <td key={colIdx} style={tdStyle}>{formatCell(row[colIdx], colIdx, activeTab, globalSearch, columnFilters[colIdx])}</td>)}
                </tr>
              ))}
            </tbody>
          </table>
...
function formatCell(value, colIdx, tab, globalSearch, colFilter) {
  if (!value) return <span style={{ color: '#ccc' }}>—</span>;
  // Meet link
  if (tab === 'sessions' && colIdx === 13 && value.startsWith('http')) {
    return <a href={value} target="_blank" rel="noopener noreferrer" style={{ color: '#2563eb' }}>Join Meet</a>;
  }
